...
function lintModuleVariablesNewline(node, context, moduleType) {
  const minProperties = context?.options?.find(op => op.minProperties)?.minProperties || 2;
  ...
  for (let i = 1; i < moduleVariables.length; i++) {
    ...
    if (moduleVariables[i].loc.start.line === moduleVariables[i - 1].loc.start.line) {
      ...
      if (namedImportAfterDefault) {
        if (moduleVariables.length <= minProperties) {
          return null;
        }
        const endOfDefaultImport = node.specifiers[i - 1].range[minProperties - 1];
        ...
        const brace = sourceCode.tokensAndComments.find(
          (token) => token.type === "Punctuator"
            ...
            && token.range[minProperties - 1] <= beginningOfNamedImport
        );
        const rangeAfterBrace = [brace.range[0], brace.range[minProperties - 1]];
